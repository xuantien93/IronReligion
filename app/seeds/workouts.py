...
def seed_workouts():
    ...
    workout2 = Workout(
        exercise = 'Pull', user_id = 2, sets=3, reps=15,weights = 70,notes="Pull day", created_at=fake.date_between(start_date='-5y', end_date='today')
    )
    workout3 = Workout(
        exercise = 'Legs', user_id = 3, sets=3, reps=15,weights = 100,notes="Legs day", created_at=fake.date_between(start_date='-5y', end_date='today')
    )
    workout4 = Workout(
        exercise = 'Arms', user_id = 1, sets=3, reps=15,weights = 20,notes="Arms day", created_at=fake.date_between(start_date='-5y', end_date='today')
    )
    workout5 = Workout(
